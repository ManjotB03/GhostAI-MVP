...
export default function ATSCheckerPage() {
  ...
  async function handleCheck() {
    ...
    try {
      ...
      if (res.status === 429) {
        // Rate limit is a conversion moment, not an error.
        setRateLimited(true);
        return;
      }
    ...
    <main className="min-h-screen bg-slate-950 text-white">
        ...
        <div className="max-w-6xl mx-auto px-6 py-3 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2">
            <Image
              ...
              unoptimized
            />
            <span className="text-xl font-bold text-white">GhostAI</span>
          </Link>

          <Link
            href="/signup"
            className="bg-sky-500 hover:bg-sky-600 transition px-5 py-2 rounded-lg font-semibold text-white shadow-lg shadow-sky-500/20"
          >
            Sign up free
          </Link>
        </div>
      ...
      <section className="max-w-4xl mx-auto px-6 pt-16 text-center">
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-12">
          <span className="bg-slate-900 border border-slate-700 px-4 py-2 rounded-full text-sm text-slate-300">
            Free
          </span>
      ...
      <section className="max-w-6xl mx-auto px-6 pb-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <textarea
            ...
            className="min-h-[350px] rounded-2xl bg-slate-900 border border-slate-700 p-5 text-white outline-none focus:border-sky-500"
          />
        ...
        <button
          onClick={handleCheck}
          disabled={loading}
          className="w-full mt-6 bg-sky-500 hover:bg-sky-600 disabled:opacity-70 transition py-4 rounded-2xl font-bold text-lg text-white shadow-lg shadow-sky-500/20"
        >
        ...
        {/* Rate limited -> signup prompt */}
        {rateLimited && (
          <div className="mt-6 rounded-2xl border border-sky-500/30 bg-sky-500/10 p-6 text-center">
            <p className="text-lg font-semibold text-white mb-2">
              You&apos;ve used your free checks for now
            </p>
            <p className="text-slate-300 mb-5">
              Create a free account to keep checking, get full CV rewrites, and
              track every application in one place.
            </p>
            <Link
              href="/signup"
              className="inline-block bg-sky-500 hover:bg-sky-600 transition px-7 py-3 rounded-xl font-bold text-white"
            >
              Create free account →
            </Link>
          </div>
        ...
        {/* RESULTS */}
        {results && (
          <div className="mt-12 bg-slate-900/70 border border-slate-800 rounded-3xl p-8">
            <div className="flex justify-center mb-10">
              <div
                className={`w-40 h-40 rounded-full ${scoreColor} flex items-center justify-center text-5xl font-extrabold text-slate-950`}
              >
                {results.score}%
              </div>
            </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {results.missingKeywords?.length ? (
                    results.missingKeywords.map((item: string) => (
                      <span
                        key={item}
                        className="bg-red-500/15 border border-red-500/30 text-red-300 px-3 py-2 rounded-full text-sm"
                      >
                        ✕ {item}
                      </span>
                    ))
                ...
                <div className="flex flex-wrap gap-2">
                  {results.matchedKeywords?.length ? (
                    results.matchedKeywords.map((item: string) => (
                      <span
                        key={item}
                        className="bg-emerald-500/15 border border-emerald-500/30 text-emerald-300 px-3 py-2 rounded-full text-sm"
                      >
                        ✓ {item}
                      </span>
                    ))
              ...
              <div className="mt-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {results.improvements.map((item: string, index: number) => (
                    ...
                    >
                      <div className="text-sky-400 text-2xl font-bold mb-3">
                        {index + 1}
                      </div>
            ...
            <div className="mt-12 text-center">
              <Link
                href="/signup"
                className="inline-block bg-sky-500 hover:bg-sky-600 transition px-8 py-4 rounded-2xl font-bold text-white text-lg shadow-lg shadow-sky-500/20"
              >
                Want a full CV rewrite? Try GhostAI free →
              </Link>
              <p className="text-sm text-slate-400 mt-3">
                Free account also unlocks application tracking and interview prep.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="rounded-3xl border border-slate-800 bg-slate-900/70 p-8">
          ...
          <div className="space-y-5 text-slate-300 leading-relaxed">
            ...
            <p>
              ...
              &ldquo;trick&rdquo; ATS systems, but to make your CV clearer, more
              relevant, and easier for both software and recruiters to understand.
            </p>
          </div>
        </div>
      </section>
            ...
            mainEntity: [
              {
                ...
                acceptedAnswer: {
                  "@type": "Answer",
                  text: "A good ATS score is usually 70% or higher. This suggests your CV closely matches the job description and includes many relevant keywords.",
                },
              },
              {
                "@type": "Question",
                ...
                acceptedAnswer: {
                  "@type": "Answer",
                  text: "Yes. GhostAI's ATS CV checker is free to use and does not require signup to get your initial score and keyword feedback.",
                },
              },
              {
                "@type": "Question",
                name: "Should I tailor my CV for every job?",
                acceptedAnswer: {
                  "@type": "Answer",
                  text: "Yes. Tailoring your CV for each job can improve keyword alignment and make your experience more relevant.",
                },
              },
              {
                "@type": "Question",
                name: "Can ATS software reject my CV automatically?",
                acceptedAnswer: {
                  "@type": "Answer",
                  text: "Some employers use ATS software to rank, filter, or organise CVs before recruiters review them.",
                },
              },
              {
                "@type": "Question",
                name: "Does GhostAI guarantee interviews?",
                acceptedAnswer: {
                  "@type": "Answer",
                  text: "No. GhostAI helps improve your CV, ATS match, and interview preparation, but cannot guarantee interviews or job offers.",
                },
              },
            ],
          }),
